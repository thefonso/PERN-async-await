...
function hasOnlyValidProps(req, res, next) {
  //NOTE console.log("hasOnlyValidProps-req",req)
  //NOTE console.log("response",res)
  const { data } = req.body
  if (!data) {
    return next({
      status: 400,
...
// return error message if reservation_date is not in the correct format,
// in the past, or is on a Tuesday. Otherwise return false
function validDate(req) {
  const { reservation_date, reservation_time } = req.body.data
  console.log("validDate CALLED:")
  const today = new Date()
  console.log("TODAY: ",today)
  //why does this next line have GMT-04:00?
  const resDate = new Date(`${reservation_date} ${reservation_time} GMT-04:00`)
  console.log("NEW resDATE: ",resDate) //2023-02-12T03:00:00.000Z
  //
...
function validStatus(req, res, next) {
  //NOTE console.log("validStatus - req: ", req);
  //NOTE console.log("validStatus - res: ", res);
...
// takes a request and creates an array of errors. if array of errors is > 0, 
// returns the next function with a 400 status and a message of errors. 
// if array of errors < 0, it returns the next function
...
async function list(req, res) {
  //NOTE track list in service file also
  //console.log("Line 207 - controller - list",req.params)
  ...
  if (date) {
    ...
    //console.log("ALFONSO - Line 217 - controller calls service.list: ", x)
    //return x;
  }
}
...
async function updateStatus(req, res) {
  //NOTE console.log("req",req)
  //NOTE console.log("res",res)
...
async function update(req, res) {
  ...
  //console.log("ALFONSO reservations.constroller - UPDATE updatedRes - line 246: ", updatedRes)
